-- Create table for Brazilian banking institutions
CREATE TABLE IF NOT EXISTS bancos_brasileiros (
    id SERIAL PRIMARY KEY,
    codigo_compensacao VARCHAR(10) NOT NULL UNIQUE,
...
CREATE INDEX IF NOT EXISTS idx_bancos_codigo_compensacao ON bancos_brasileiros(codigo_compensacao);
CREATE INDEX IF NOT EXISTS idx_bancos_nome_instituicao ON bancos_brasileiros USING gin(to_tsvector('portuguese', nome_instituicao));

-- Insert all Brazilian banking institutions data
INSERT INTO bancos_brasileiros (codigo_compensacao, nome_instituicao) VALUES
...
ON CONFLICT (codigo_compensacao) DO NOTHING;

-- Create an RLS policy for read access (if needed in the future)
ALTER TABLE bancos_brasileiros ENABLE ROW LEVEL SECURITY;

-- Allow all authenticated users to read banking institutions data
